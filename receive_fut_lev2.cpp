...
udp_quote_fut_lev2::udp_quote_fut_lev2(void)
{
	m_res			= NULL;
}
...
//*********************重点接收处理level2行情部分代码************************
//***************************************************************************
void udp_quote_fut_lev2::handle_rx(struct resources* res, int pkt_buf_i, int len)
{
	...
	for (; remain_len > 0;)
	{
		...
		memcpy(&m_qt[node], ptr_udp, sizeof(efh3_2_fut_lev2));

		++m_ll_qt_count;
		remain_len -= sizeof(efh3_2_fut_lev2);
		ptr_udp += sizeof(efh3_2_fut_lev2);
...
bool udp_quote_fut_lev2::init(sock_udp_param &shfe)
{
	m_udp_param			= shfe;
...
int udp_quote_fut_lev2::sl_parse(ef_filter_spec* fs, const char* ip, unsigned short port)
{
...
struct pkt_buf* udp_quote_fut_lev2::pkt_buf_from_id(struct resources* res, int pkt_buf_i)
{
	assert((unsigned) pkt_buf_i < (unsigned) res->pkt_bufs_n);
	return (pkt_buf*) ((char*) res->pkt_bufs + (size_t) pkt_buf_i * PKT_BUF_SIZE);
}

void udp_quote_fut_lev2::pkt_buf_free(struct resources* res, struct pkt_buf* pkt_buf)
{
...
void udp_quote_fut_lev2::handle_rx_discard(struct resources* res, int pkt_buf_i, int len, int discard_type)
{
...
void udp_quote_fut_lev2::refill_rx_ring(struct resources* res)
{
	struct pkt_buf* pkt_buf;
	int i;
...
void udp_quote_fut_lev2::thread_main_loop(struct resources* res)
{
	ef_event evs[32];
...
void* udp_quote_fut_lev2::func_work_thread(void* param)
{
	udp_quote_fut_lev2* ptr = (udp_quote_fut_lev2*) param;
	ptr->on_work_thread();

	return 0;
}

int udp_quote_fut_lev2::on_work_thread()
{
...
void udp_quote_fut_lev2::write_log(const char* msg)
{
	cout << msg << endl;
}

bool udp_quote_fut_lev2::bind_cpu( int cpu_id, pthread_t thd_id )
{
...
bool udp_quote_fut_lev2::start_recv_thread()
{
	pthread_t	id;
	...
	if ( 0 != pthread_create(&id, &threadAttr, udp_quote_fut_lev2::func_work_thread, (void*)(this)) )
	{
...
void udp_quote_fut_lev2::log_quote()
{
	...
	sprintf( str_full_name, "%04d%02d%02d_fut_lev2.csv", ltm->tm_year + 1900, ltm->tm_mon + 1, ltm->tm_mday );
